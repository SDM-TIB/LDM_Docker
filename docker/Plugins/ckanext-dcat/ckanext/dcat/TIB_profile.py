...
class TIBDCATAPProfile(RDFProfile):
    '''
    ...
    def graph_from_dataset(self, dataset_dict, dataset_ref):
        ...
        # Main Author
            ...
            self._add_triple_from_dict(dataset_dict, creator, FOAF.Name, 'author')
            orcid = self._get_dataset_value(dataset_dict, 'orcid')
            if orcid:
                orcid_uri = CleanedURIRef("http://orcid.org/" + orcid)
                self.g.add((creator, DCT.identifier, orcid_uri))
            g.add((dataset_ref, DCAT.creator, creator))


            # Other authors
            extra_authors = self._get_dataset_value(dataset_dict, 'extra_authors')
            if extra_authors:
                for author in extra_authors:
                    creator_ref = DCT.creator
                    creator = DCT.Agent
                    ...
                    g.add((dataset_ref, DCAT.creator, creator_ref))
        ...
        for resource_dict in dataset_dict.get('resources', []):
            ...
            if dataset_dict.get('type') == 'service':
                if 'datasets_served' in dataset_dict:
