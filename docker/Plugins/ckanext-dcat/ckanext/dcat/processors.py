...
DEFAULT_RDF_PROFILES = ['tib_dcat_ap']
...
class RDFSerializer(RDFProcessor):
    ...
    def graph_from_dataset(self, dataset_dict):
        ...
        if dataset_dict.get('doi'):
            uri_ref = 'https://doi.org/'+dataset_dict.get('doi')
        else:
            uri_ref = dataset_uri(dataset_dict)
            if dataset_dict.get('type') == 'service':
                uri_ref = uri_ref.replace('/dataset/', '/service/')

        dataset_ref = URIRef(uri_ref)
