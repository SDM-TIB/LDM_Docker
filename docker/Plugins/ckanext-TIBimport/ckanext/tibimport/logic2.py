...
class LDM_DatasetImport:
    ...
    def __init__(self, ds_parser, logged_user=""):
        # Dataset Parser
        self.ds_parser  = ds_parser
#
        if logged_user=="":
            self.logged_user = flask_d.user # Comment for testing and counterwise
        else:
            self.logged_user = logged_user # used for testing
    ...
    def get_local_organization(self, name):
        ...
        try:
            result = toolkit.get_action('organization_show')(context, params)
        except NotFound as e:
            return {}
    ...
    def insert_dataset(self, ds_dict):
        ...
        context = {'model': model,
                   'session': model.Session,
                   'user': self.logged_user }
            #                   'auth_user_obj': flask_d.userobj}
        toolkit.get_action('package_create')(context, ds_dict)
    ...
    def update_dataset(self, ds_dict):
        ...
        context = {'model': model,
                   'session': model.Session,
                   'user': self.logged_user}
        toolkit.get_action('package_update')(context, ds_dict)
    ...
    def delete_dataset(self, name):
        ...
        context = {'model': model,
                   ...
                   'user': self.logged_user}
        ds_dict = {"id": name}
        toolkit.get_action('package_delete')(context, ds_dict)
    ...
    def delete_organization(self, name):
        ...
        context = {'model': model,
                   ...
                   'user': self.logged_user}
        org_dict = {"id": name}
        toolkit.get_action('organization_delete')(context, org_dict)
    ...
    def import_datasets(self):
        ...
        #self._insert_update_skip_dataset(remote_datasets[1])
    ...
    def _insert_update_skip_dataset(self, remote_dataset):
        ...
        elif dataset['source_metadata_modified'] == remote_dataset['metadata_modified']:
...
class DatasetParser():
    ...
    def _config_logger(self):
        ...
        log_file = self.log_file_prefix+date.today().strftime("%Y_%m_%d")+"_log.log"
    ...
    def get_summary_log(self):
        summary_log = "SUMMARY\n"
        summary_log += "Datasets inserted: " + str(self.logger.datasets_inserted) + "\n"
        summary_log += "Datasets updated: " + str(self.logger.datasets_modified) + "\n"
        summary_log += "Datasets skiped: " + str(self.logger.datasets_skiped) + "\n"
        return summary_log
