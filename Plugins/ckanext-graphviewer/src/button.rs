...
pub fn fetch_author_datasets(
    ...
    author_id: String, // NEW: Using ID instead of ORCID
) {
            ...
            if let Some(text) = res.text() {
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&text) {
                    ...
                    if let AppState::Ready { raw_triples, nodes, edges, .. } = &mut *state_lock {
                        ...
                        // --- FIX 1: BULLETPROOF HIERARCHY ---
                        // Instead of checking if nodes are "old", we explicitly check their RDF Type.
                        // If a Dataset points to an Author, we FORCE it to flip so Author -> Dataset!
...
// --- FIX 2: IDENTIFY ALL 1-HOP CONNECTIONS TO THE AUTHOR ---
                        ...
                        for (i, n) in new_nodes.iter_mut().enumerate() {
                            if let Some(old_n) = old_nodes.get(&n.id) {
                                // OLD NODE: Preserve its exact position!
                                n.pos = old_n.pos;
